...
function drawACard(operator) {
    let drawnCard = cardStapel[Math.floor(Math.random() * cardStapel.length)];
    const backCard = document.getElementById("deck")
    const duration = 1000;
    let startTime = null;
    if (operator === "player"){
        // Position der Playersachen
        const targetX = 150
        const targetY = 700
        const startX = 0
        const startY = 0
...
function deckOfCards() {
    ...
    cardsOfDeck.src ='imagescards/back_light.png'
...
function player1(descision){
    if (descision === "hit"){
        drawnCardPlayer.push(drawACard('player')) // Drawing a card and Adding it to deck
